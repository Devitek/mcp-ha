...
export function effectiveTokens(cfg: AddonConfig): ApiToken[] {
  // Since 1.3.0 (#182) the primary token is the only config-side token;
  // every other token lives hashed in the store with fine-grained grants.
  return [{ name: "default", token: cfg.apiToken, scope: "write" }];
}
