...
- **`tokens.db: filesystem locks look broken` in the log**: something denies the fcntl locks SQLite asks for. Before 1.0.3 the add-on's own AppArmor profile was the culprit (fixed; update if you still run an older version). The message can still appear legitimately on network mounts and some overlays: the add-on then reopens the token store WITHOUT SQLite locking, which is safe here because the Supervisor runs a single instance, and the log line names the detected filesystem. If `/data` sits on a network share, moving it to local storage restores normal locking (and the faster WAL journal). If you also see `falling back to an IN-MEMORY store`, the database could not open at all: check that log line and open an issue with it.
