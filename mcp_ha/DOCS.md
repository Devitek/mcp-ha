...
- **`tokens.db: filesystem locks look broken` in the log**: the filesystem hosting `/data` never grants the fcntl locks SQLite asks for (typical of network mounts and some overlays). The add-on automatically reopens the token store WITHOUT SQLite locking, which is safe here because the Supervisor runs a single instance; the log line names the detected filesystem. Everything works, but if `/data` sits on a network share, moving it to local storage restores normal locking (and the faster WAL journal). If you also see `falling back to an IN-MEMORY store`, the database could not open at all: check that log line and open an issue with it.
