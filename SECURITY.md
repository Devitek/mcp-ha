...
2. **Read only by default.** With `allow_write: false`, none of the four write tools (`ha_call_service`, `ha_run_script`, `ha_trigger_automation`, `ha_set_automation`) is registered at all: they do not even appear in the client's tool list.
