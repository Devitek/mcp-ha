...
2. **Read only by default.** With `allow_write: false`, none of the nine write tools (`ha_call_service`, `ha_run_script`, `ha_trigger_automation`, `ha_set_automation`, `ha_create_helper`, `ha_delete_helper`, `ha_send_notification`, `ha_snapshot_scene`, `ha_announce`) is registered at all: they do not even appear in the client's tool list. Helper creation is deliberately the mildest write there is: helpers are pure state containers with no behaviour, and creating one cannot make the house act (#94 tier 1); deletion additionally honours the entity lists.
