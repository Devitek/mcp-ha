...
Changements d'état d'une entité. Fenêtre : `hours` (défaut 24, max 168) ou `start`/`end` en ISO 8601. Au-delà de 500 points, sous-échantillonnage avec une note.
...
Événements lisibles, filtrables par `entity_id`, fenêtre jusqu'à 7 jours, plafonné à 200 événements.
