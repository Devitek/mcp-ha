...
| `api_tokens` | `[]` | **Dépréciée depuis 1.2.0.** Jetons nommés legacy avec une portée : toujours acceptés comme source d'import one-shot (hachés dans le store au démarrage), puis l'option est vidée ; la clé elle-même doit rester tant que le schéma la déclare (règle Supervisor) et disparaîtra avec le schéma dans une release future. Gérez les jetons depuis la page ingress. |
