...
| `api_tokens` | `[]` | **Deprecated since 1.2.0.** Legacy named tokens with a scope: still accepted as a one-shot import source (hashed into the token store at boot), then the key is removed from the stored options. Manage tokens from the ingress page; the option will be dropped in a future release. |
