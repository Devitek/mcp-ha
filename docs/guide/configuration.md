...
| `api_tokens` | `[]` | **Deprecated since 1.2.0.** Legacy named tokens with a scope: still accepted as a one-shot import source (hashed into the token store at boot), then blanked; the key itself must stay while the schema declares it (Supervisor rule) and will be dropped with the schema in a future release. Manage tokens from the ingress page. |
...
The single `api_token` grants full access (it is the bootstrap and recovery token). Named tokens are stored **hashed** in a local database. The YAML `api_tokens` option is **deprecated**: entries added there are still imported once at the next start (with the grants their scope implies), then the option is blanked, with a deprecation warning in the log. The key itself disappears together with the schema in a future release. Creating tokens happens on the **ingress page** (sidebar entry), Tokens tab: pick a level (none / read / write / manage) per tool category, optionally an expiry and per-token entity lists (applied on writes, on top of the global ones). The secret is shown once at creation; grants are capped by the `allow_*` options on every request, so closing a gate instantly degrades every token. Revocation is one click.

Deprecated legacy form, still accepted as a one-shot import source (`scope: read` maps to read-everywhere grants, `scope: write` to full grants, both capped by the `allow_*` gates):
